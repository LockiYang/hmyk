...
class Pay extends Frontend
{
    ...
    public function goods()
    {
        ...
        try {
            ...
            db::name('goods_order')->insert($order_insert);
    ...
    protected function notifyGoodsSuccess($goods, $out_trade_no)
    {
        ...
        if ($goods['type'] == 'fixed') {
            ...
            db::name('deliver')->insertAll($deliver);
            //更新库存表
            db::name('stock')->where(['id' => $stock['id']])->setDec('num', $order['goods_num']);
            doAction('send_goods', $order, $deliver);
        }
        ...
        //更新商品库存字段
        db::name('goods')->where(['id' => $goods['id']])->setDec('stock', $order['goods_num']);
        //更新商品销量字段
        db::name('goods')->where(['id' => $goods['id']])->setInc('sales', $order['goods_num']);
        if ($goods['invented_sales']) {
            //更新商品虚拟销量字段
            db::name('goods')->where(['id' => $goods['id']])->setInc('invented_sales', $order['goods_num']);
        }
        //更新订单状态
        db::name('goods_order')->where(['id' => $order['id']])->update(['pay_time' => $this->timestamp]);
        //更新库存表
        db::name('sku')->where(['id' => $order['sku_id']])->setDec('stock', $order['goods_num']);
